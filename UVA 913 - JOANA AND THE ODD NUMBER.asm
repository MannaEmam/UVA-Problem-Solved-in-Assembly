;UVA 913 - JOANA AND THE ODD NUMBER
ORG 100H 
.DATA
N DW ?
K DW ?
...
CALL INPUT;TAKING N
MOV N,AX
MOV K,1;STORING 1 IN K
MOV BX,3;STORING 1 IN BX
FOR1:
MOV CX,0;STORING 0 IN CX
CMP BX,N;IF BX > N?
JG NEXT;YES GO TO NEXT
;ELSE
FOR2:
ADD K,2;ADDING 2 EVERYTIME THE LOOP EXECUTE
INC CX;INCREMENTING CX
CMP CX,BX;IF CX <BX ?
JL  FOR2;YES THEN LOOP
;ELSE
ADD BX,2
JMP  FOR1
NEXT:
XOR AX,AX;CLEARING AX
ADD AX,K;ADDING SUM AND K
SUB K,2;GETTING THE 2ND LAST ODD NUMBER
ADD AX,K;ADDING SUM AND K
SUB K,2;GETTING THE 3RD LAST ODD NUMBER
ADD AX,K;ADDING SUM AND K
CALL OUTPUT
JMP TOP
    RET
