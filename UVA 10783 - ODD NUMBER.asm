...
MOV C,1;STORING 1 IN C 
   ...
   TOP:
   MOV R,0;STORING 0 IN R  
   CALL INPUT;TAKING A
   MOV A,AX;STORING AX IN A
   CALL INPUT;TAKING B
   MOV B,AX;STORING AX IN B
...
CALL OUTPUT
LOOP1:
MOV AX,A;STORING A IN BX
CWD
MOV BX,2
DIV BX
CMP DX,0;CHECKING EVEN OR ODD
JZ NEXT;IF EVEN THEN GO TO NEXT
;ELSE
MOV BX,A;STORING A IN BX
ADD R,BX;R += A
NEXT:
INC A;INCREMENTING A
MOV BX,A;STORING INCREMENTED VALUE IN BX
CMP BX,B;IF A <= B
JG LAST;THEN GO TO LAST
;ELSE
JMP LOOP1
LAST:
